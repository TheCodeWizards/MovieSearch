...
const PopularTv = () => {
    ...
    useEffect(() => {
        const url = `https://api.themoviedb.org/3//tv/popular?language=en-US&page=${pageNo}`;
        const fetchPopularTv = async () => {
            try {
                const res = await fetch(url, options);
                const data = await res.json();
                setPopularTv((prevData) => [...prevData, ...data.results]);
            } catch (err) {
    ...
    if (!popularTv) return <Loader />
    if (popularTv.length === 0) return <Loader />
