...
const AirToday = () => {
    ...
    useEffect(() => {
        const url = `https://api.themoviedb.org/3//tv/on_the_air?language=en-US&page=${pageNo}`;
        const fetchAirToday = async () => {
            try {
                const res = await fetch(url, options);
                const data = await res.json();
                setAirToday((prevData) => [...prevData, ...data.results]);
            } catch (err) {
    ...
    // if (!airToday) return 
    if (airToday.length === 0) return <Loader />
